...
ADR6        & Table SAP des adresses email (résolution destinataire) \\
...
\noindent\hyperref[ch:etude-analyse]{Chapitre 3 : \'{E}tude et analyse de l'existant (Processus cibles)} \dotfill \pageref*{ch:etude-analyse}\\
\noindent\hspace*{1em}\hyperref[ch3:introduction]{3.1 Introduction} \dotfill \pageref*{ch3:introduction}\\
\noindent\hspace*{1em}\hyperref[ch3:analyse]{3.2 Analyse de l'existant} \dotfill \pageref*{ch3:analyse}\\
\noindent\hspace*{2em}\hyperref[ch3:confirmation]{3.2.1 Confirmation des lignes de commande difficile \`{a} g\'{e}rer} \dotfill \pageref*{ch3:confirmation}\\
\noindent\hspace*{2em}\hyperref[ch3:masse]{3.2.2 Absence de traitement en masse via fichiers} \dotfill \pageref*{ch3:masse}\\
\noindent\hspace*{2em}\hyperref[ch3:rejet]{3.2.3 Rejet des lignes et justification non structur\'{e}s} \dotfill \pageref*{ch3:rejet}\\
\noindent\hspace*{2em}\hyperref[ch3:replanification]{3.2.4 Replanification des dates de livraison non g\'{e}r\'{e}e par le fournisseur} \dotfill \pageref*{ch3:replanification}\\
\noindent\hspace*{1em}\hyperref[ch3:besoins]{3.3 \'{E}tude des besoins} \dotfill \pageref*{ch3:besoins}\\
\noindent\hspace*{2em}\hyperref[ch3:besoins-f]{3.3.1 Besoins fonctionnels du projet} \dotfill \pageref*{ch3:besoins-f}\\
\noindent\hspace*{2em}\hyperref[ch3:besoins-nf]{3.3.2 Besoins non fonctionnels du projet} \dotfill \pageref*{ch3:besoins-nf}\\
\noindent\hspace*{1em}\hyperref[ch3:solution]{3.4 Pr\'{e}sentation de la solution envisag\'{e}e} \dotfill \pageref*{ch3:solution}\\
\noindent\hspace*{2em}\hyperref[ch3:rapport]{3.4.1 Rapport de confirmation des commandes d'achat} \dotfill \pageref*{ch3:rapport}\\
\noindent\hspace*{2em}\hyperref[ch3:confirm-manuel]{3.4.2 Confirmation manuelle des lignes} \dotfill \pageref*{ch3:confirm-manuel}\\
\noindent\hspace*{2em}\hyperref[ch3:confirm-masse]{3.4.3 Confirmation en masse via fichier Excel} \dotfill \pageref*{ch3:confirm-masse}\\
\noindent\hspace*{2em}\hyperref[ch3:rejet-comm]{3.4.4 Rejet de lignes et ajout de commentaires} \dotfill \pageref*{ch3:rejet-comm}\\
\noindent\hspace*{2em}\hyperref[ch3:maj-date]{3.4.5 Mise \`{a} jour de la date de livraison} \dotfill \pageref*{ch3:maj-date}\\
\noindent\hspace*{2em}\hyperref[ch3:affichage-rejet]{3.4.6 Affichage de toutes les commandes d'achat rejet\'{e}es} \dotfill \pageref*{ch3:affichage-rejet}\\
...
\noindent\hspace*{1em}\hyperref[ch4:ia-joule]{4.7 Stratégie d'automatisation et Agent IA (SAP Joule)} \dotfill \pageref*{ch4:ia-joule}\\
\noindent\hspace*{1em}\hyperref[ch4:conclusion]{4.8 Conclusion} \dotfill \pageref*{ch4:conclusion}\\
...
Le present rapport retrace les differentes etapes de ce projet, organise en cinq parties :

\begin{itemize}[label=---,itemsep=0.2em,topsep=0.2em]
\item \textbf{Chapitre I : Contexte general du projet}

Presentation de l'organisme d'accueil, l'environnement metier et technologique du projet.

\item \textbf{Chapitre II : Etude et analyse de l'existant}

Analyse approfondie des systemes ERP, focus sur SAP et le module MM pour la gestion des matieres.

\item \textbf{Chapitre III : Etude et analyse de l'existant (Processus Cibles)}

Analyse des processus actuels de gestion des commandes fournisseurs, identification des limites des methodes traditionnelles, et etude des besoins fonctionnels et non fonctionnels.

\item \textbf{Chapitre IV : Analyse et conception de la solution}

Conception fonctionnelle et technique du portail, choix des outils SAP (CDS, RAP, Fiori, etc.) et modelisation des flux de donnees.

\item \textbf{Chapitre V : Developpement et realisation}

Presentation des technologies utilisees, developpement du portail, demonstration des cas d'usage (confirmation, rejet, IA, analytique), tests et validation.
\end{itemize}
...
\chapter{\'Etude et analyse de l'existant}\label{ch:etude-analyse}
\section{Introduction}\label{ch3:introduction}

Dans ce chapitre, nous allons procéder à une analyse détaillée de l'existant, en mettant en lumière les mécanismes utilisés actuellement pour le suivi et la validation des commandes d'achat. Cette analyse nous permettra d'identifier les limitations, lenteurs et zones de rupture dans le processus actuel.

Nous allons ensuite étudier les besoins fonctionnels et non fonctionnels du projet pour bien comprendre les spécifications et satisfaire le client.

Enfin, nous présenterons la solution fonctionnelle envisagée, qui s'appuie sur les dernières technologies SAP (ABAP RAP, CDS Views, Fiori), ainsi que les règles de gestion définies par le client, dans le but de répondre aux enjeux identifiés et de renforcer l'efficacité et la collaboration entre l'entreprise et ses fournisseurs.

\section{Analyse de l'existant}\label{ch3:analyse}

Actuellement, le processus de confirmation des commandes d'achat dans le système SAP standard s'effectue principalement par le biais d'interactions manuelles ou par intégration EDI dans certains cas avancés. Toutefois, aucune interface utilisateur conviviale n'est mise à disposition des fournisseurs pour gérer en autonomie leurs commandes, ce qui introduit plusieurs limitations opérationnelles et organisationnelles.

\subsection{Confirmation des lignes de commande difficile à gérer}\label{ch3:confirmation}

La confirmation des lignes de commande (partielle ou totale) nécessite aujourd'hui une intervention manuelle des acheteurs dans SAP, via les transactions classiques (ME22N, ME23N), sans que le fournisseur puisse intervenir directement. Cela génère :

\begin{itemize}[label=\textbullet, itemsep=0.2em]
  \item Une charge de travail importante pour les acheteurs,
  \item Un manque de traçabilité claire sur qui a confirmé quoi et quand,
  \item Des retards dans la mise à jour des délais de livraison.
\end{itemize}
...
\subsection{Absence de traitement en masse via fichiers}\label{ch3:masse}

Le système actuel n'intègre pas nativement une interface d'importation Excel permettant au fournisseur de soumettre plusieurs confirmations de lignes en une seule opération. Cela limite la capacité des fournisseurs à traiter des volumes importants efficacement, et alourdit considérablement le processus.

\subsection{Rejet des lignes et justification non structurés}\label{ch3:rejet}

Il n'existe pas de mécanisme intégré permettant au fournisseur de rejeter certaines lignes de commande avec des commentaires justifiant ce rejet. Les causes de refus (produit indisponible, délai irréaliste, etc.) ne sont donc pas tracées dans le système, ce qui complique :

\begin{itemize}[label=\textbullet, itemsep=0.2em]
  \item La compréhension des motifs de rejet,
  \item La réactivité des acheteurs pour corriger ou replanifier les commandes,
  \item La communication fluide entre les deux parties.
\end{itemize}
...
\subsection{Replanification des dates de livraison non gérée par le fournisseur}\label{ch3:replanification}

Dans le modèle actuel, toute modification de la date de livraison doit être communiquée manuellement au service achat, qui devra alors la modifier dans SAP. Cela ralentit fortement la prise en compte de nouvelles contraintes logistiques, et peut engendrer des désalignements entre les dates réelles et les dates prévues dans le système.

\section{\'Etude des besoins}\label{ch3:besoins}

\subsection{Besoins fonctionnels du projet}\label{ch3:besoins-f}

Notre projet doit répondre principalement aux besoins fonctionnels suivants :
...
\subsection{Besoins non fonctionnels du projet}\label{ch3:besoins-nf}

Les besoins non fonctionnels définissent les critères de qualité que la solution doit respecter pour garantir la performance, la sécurité et la maintenabilité.

\begin{itemize}[label=\textbullet, itemsep=0.2em]
  \item \textbf{Sécurité des données} : Authentification via rôles SAP et gestion des autorisations.
  
  \item \textbf{Performance} : L'affichage des listes et le traitement des fichiers doivent être rapides et adaptés aux grands volumes de données.
  
  \item \textbf{Intégration native SAP} : La solution doit être intégrée au système SAP S/4HANA sans outils tiers, via ABAP RAP et Fiori. Respect des standards SAP et utilisation des services CDS.
  
  \item \textbf{Ergonomie} : L'interface doit être intuitive, fluide et responsive, facilitant la prise en main par des fournisseurs externes.
  
  \item \textbf{Traçabilité et auditabilité} : Toutes les modifications doivent être traçables dans le système SAP, avec possibilité de consulter les historiques.
  
  \item \textbf{Modularité et évolutivité} : La solution doit être conçue de manière modulaire pour permettre l'ajout futur de fonctionnalités.
\end{itemize}
...
\subsection{Rapport de confirmation des commandes d'achat}\label{ch3:rapport}

Le portail affichera un rapport dynamique listant toutes les commandes d'achat adressées au fournisseur connecté. Ce rapport inclura les informations essentielles de chaque ligne de commande (poste), telles que : le numéro de commande, le code article et la description, la quantité commandée, la date de livraison demandée, le prix unitaire, le statut de confirmation...

Chaque fournisseur n'aura accès qu'à ses propres commandes, garantissant la confidentialité des données.

\subsection{Confirmation manuelle des lignes (partielle ou totale)}\label{ch3:confirm-manuel}

Le fournisseur pourra confirmer une ou plusieurs lignes d'une commande directement depuis l'interface :

\begin{itemize}[label=\textbullet, itemsep=0.2em]
  \item En cliquant sur un bouton ``Confirm as Ordered'', les lignes sélectionnées seront automatiquement mises à jour dans SAP avec le statut de confirmation correspondant.
  \item En cas de confirmation partielle, seules les lignes concernées seront impactées.
  \item Ces actions seront traitées en mode transactionnel via EML (Entity Manipulation Language) et les changements seront persistés via les CDS views et les Behaviors ABAP RAP.
\end{itemize}

\subsection{Confirmation en masse via fichier Excel}\label{ch3:confirm-masse}

Pour les fournisseurs gérant un grand volume de commandes, une fonctionnalité permettra de :

\begin{itemize}[label=\textbullet, itemsep=0.2em]
  \item Télécharger un modèle Excel contenant les lignes de commandes à traiter,
  \item Le remplir hors ligne avec les données de confirmation (quantité confirmée, date mise à jour, etc.),
  \item Et le téléverser dans le portail pour traitement automatique.
\end{itemize}

Les confirmations seront alors analysées ligne par ligne, et chaque résultat sera affiché (confirmé, rejeté, ou en erreur), avec possibilité de corriger et re-téléverser le fichier.

\subsection{Rejet de lignes et ajout de commentaires}\label{ch3:rejet-comm}

Le fournisseur pourra rejeter une ou plusieurs lignes en indiquant un motif de refus via un champ commentaire dédié. Par exemple :

\begin{itemize}[label=\textbullet, itemsep=0.2em]
  \item Produit non disponible,
  \item Délai non acceptable,
  \item Conditions commerciales non validées, etc.
\end{itemize}

Chaque rejet sera enregistré dans le système et automatiquement notifié à l'acheteur concerné dans SAP. Ces commentaires seront visibles via un rapport d'analyse ou une notification spécifique, facilitant la réactivité du service achat.

\subsection{Mise à jour de la date de livraison (replanification)}\label{ch3:maj-date}

En cas d'impossibilité de respecter la date de livraison initiale, le fournisseur pourra proposer une nouvelle date directement depuis le portail. Le système enregistrera cette nouvelle date tout en conservant l'historique de la date originale.

\subsection{Affichage de toutes les commandes d'achat rejetées}\label{ch3:affichage-rejet}

Le fournisseur aura la possibilité d'afficher toutes les commandes rejetées avec leurs détails et les raisons, en ayant la possibilité de faire une recherche, trier, grouper, sélectionner avec une aide à la recherche.

\section{Conclusion}\label{ch3:conclusion}

L'analyse de l'existant nous a permis de mieux comprendre le fonctionnement actuel du processus de confirmation des commandes fournisseurs au sein de l'environnement SAP MM. Cette étude a mis en évidence plusieurs limites, notamment l'absence d'un outil centralisé et intuitif pour les fournisseurs, les risques d'erreurs manuelles dans la communication des confirmations, ainsi que le manque de visibilité pour les acheteurs sur les motifs de rejet ou les modifications proposées.

Les besoins exprimés par les utilisateurs finaux et les contraintes identifiées ont servi de fondement à la définition d'une solution adaptée. Celle-ci devra garantir l'autonomie des fournisseurs dans la gestion de leurs commandes, tout en assurant une traçabilité complète et une interaction fluide avec le système SAP. Dans le chapitre suivant, nous présenterons la conception détaillée de cette solution, en précisant les choix techniques et fonctionnels qui guideront sa mise en œuvre.
...
\IfFileExists{figures/sequence-diagram.png}{%
    ...
    \fbox{\parbox[c][6cm][c]{0.9\linewidth}{\centering \small Diagramme de séquence global\\(ajoutez figures/sequence-diagram.png)}}
}
\caption{Diagramme de séquence global des flux transactionnels}
\label{fig:sequence-global}
\end{figure}
