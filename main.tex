...
Une fois les données traitées massivement via l'Entity Manipulation Language (EML), le système renvoie un rapport détaillé. La figure~\ref{fig:demo-masse-succes} illustre le résultat de cette exécution, affichant les lignes intégrées avec succès ainsi que les éventuelles erreurs détectées, évitant un travail fastidieux de saisie manuelle.
...
\IfFileExists{figures/demo-masse-succes.png}{%
    ...
    \fbox{\parbox[c][5cm][c]{0.85\linewidth}{\centering \small Capture d'écran : Rapport d'exécution de masse (Succès/Erreurs)\\ (ajoutez figures/demo-masse-succes.png)}}
}
...
\IfFileExists{figures/demo-imprimer.png}{%
    ...
    \fbox{\parbox[c][6cm][c]{0.85\linewidth}{\centering \small Capture d'écran : Téléchargement PDF / Adobe Forms\\ (ajoutez figures/demo-imprimer.png)}}
}
...
Cette section illustre l'intégration de l'Intelligence Artificielle dans les processus métiers standards. Via la bulle de chat flottante de SAP Joule, le fournisseur peut formuler une requête en langage naturel, comme illustré dans la figure~\ref{fig:demo-ia-action} (par exemple : "Confirme la commande 4500001234"). Le moteur LLM hébergé sur SAP BTP analyse l'intention et orchestre l'appel.

\begin{figure}[H]
\centering
\IfFileExists{figures/demo-ia-action.png}{%
    \includegraphics[width=0.85\textwidth]{figures/demo-ia-action.png}
}{%
    \fbox{\parbox[c][5cm][c]{0.85\linewidth}{\centering \small Capture d'écran : Saisie du prompt dans Joule\\ (ajoutez figures/demo-ia-action.png)}}
}
\caption{Exécution d'une action en langage naturel avec SAP Joule}
\label{fig:demo-ia-action}
\end{figure}

Une fois l'opération validée dans le backend via le SAP Cloud Connector, Joule formule une réponse textuelle fluide et contextuelle pour confirmer le bon déroulement de l'action. La figure~\ref{fig:demo-ia-succes} présente ce retour direct dans l'interface de chat.

\begin{figure}[H]
\centering
\IfFileExists{figures/demo-ia-succes.png}{%
    \includegraphics[width=0.85\textwidth]{figures/demo-ia-succes.png}
}{%
    \fbox{\parbox[c][5cm][c]{0.85\linewidth}{\centering \small Capture d'écran : Réponse et validation par l'IA Joule\\ (ajoutez figures/demo-ia-succes.png)}}
}
\caption{Confirmation de traitement générée par l'agent IA}
\label{fig:demo-ia-succes}
\end{figure}
...
La validation de la solution a fait l'objet d'une campagne de tests rigoureuse, couvrant à la fois les aspects fonctionnels et techniques.

\begin{itemize}[label=\textbullet, itemsep=0.3em]
    \item \textbf{Tests Unitaires (Backend) :} Validation de la logique ABAP des "Behavior Implementations" pour s'assurer que les statuts de rejet et les contrôles sur les dates de livraison réagissent correctement face à des jeux de données d'erreur.
    \item \textbf{Tests d'Intégration OData :} Utilisation du \textit{SAP Gateway Client} et d'outils externes (comme Postman) pour valider les payloads JSON et les codes de statut HTTP renvoyés lors de l'import massif Excel.
    \item \textbf{Tests Utilisateurs (UAT) :} Déploiement de l'application Fiori sur l'environnement de qualification (QUAL). Les profils acheteurs et fournisseurs simulés ont parcouru l'ensemble des cas d'utilisation pour valider l'ergonomie, les temps de réponse de l'interface, ainsi que la pertinence des réponses générées par l'agent IA Joule.
\end{itemize}

L'ensemble des anomalies identifiées durant cette phase a été corrigé avant la validation finale du livrable, garantissant une application stable et prête pour une mise en production.
